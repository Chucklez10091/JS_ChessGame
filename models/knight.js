...
export default class knight extends piece{
    ...
    updateOptions(board){
        ...
        if (is_edge > 0){
            ...
            if(this.loc - 15 >= 0){temp.push(this.loc - 15);}
        }
        if (is_edge > 1){
            if(this.loc + 6 < 64){temp.push(this.loc + 6);}
            if(this.loc - 6 >= 0){temp.push(this.loc - 6);}
        }
        if (is_edge < 7){
            if(this.loc + 17 < 64){temp.push(this.loc + 17);}
            if(this.loc - 17 >= 0){temp.push(this.loc - 17);}
        }
        if (is_edge < 6){
            if(this.loc + 10 < 64){temp.push(this.loc + 10);}
            if(this.loc - 10 >= 0){temp.push(this.loc - 10);}
        }
